...
export function WorkspaceSelector({
  ...
  setSelectedDirectoryAndSelectedFile: (selectedDirectory: DirectoryNode, selectedFile:DirectoryNode) => void;
}) {
  ...
  const handleDirectorySelect = async () => {
    try {
      ...
      if(directoryNode.children.length === 0) {
        const newFile = await directoryNode.createFile("Untitled.md");
        if(!newFile) {
        ...
        await directoryNode.children[0].loadFileContent();
      }
      setSelectedDirectoryAndSelectedFile(directoryNode, directoryNode.children[0]);
    } catch (error) {
  ...
  const handleDecideLater = async () => {
    ...
    setSelectedDirectoryAndSelectedFile(emptyBrowserDirectory, emptyBrowserDirectory.children[0]);
  };
        ...
        <div className="mx-8 mt-8 mb-4">
          ...
          <div className="mt-4">
            {isSupported ? (
              ...
              <p className="text-red-700 text-sm italic pb-4">
